...
export default class FullScreenContainer extends React.Component {
    ...
    static propTypes = {
        ...
        onPhotoPressTimeout: PropTypes.number
    };
    ...
    static defaultProps = {
        ...
        delayLongPress: 1000,
        onPhotoPressTimeout: 180000 // 3分钟
        // onPhotoPressTimeout: 15000 // 10s
    };
     ...
     * @private
     */
    _setOnPhotoPressTimeout() {
        console.warn('_setOnPhotoPressTimeout');
        this._onPhotoPressTimeout && clearTimeout(this._onPhotoPressTimeout);
        this._onPhotoPressTimeout = setTimeout(() => {
            this.props.onPhotoPress();
        }, this.props.onPhotoPressTimeout)
    }
    ...
    _updatePageIndex(index) {
        this.setState({
            ...
            const newTitle = `第 ${index + 1} 张`;
            this.props.updateTitle(newTitle);

            // 无操作返回计时
            this._setOnPhotoPressTimeout();
        });
    }
        ...
        return (
            <View key={`row_${rowID}`} style={styles.flex}>
                <TouchableWithoutFeedback
                    ...
                    onPress={this._onPhotoPress.bind(this)}
